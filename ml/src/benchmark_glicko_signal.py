"""
A/B benchmark: Glicko-2 signal quality — RD decay + prestige reform.

Tests 4 Glicko computation configurations while keeping the same 4-track
split features (production). Only the underlying rating computation changes.

  Config A (baseline): no RD decay, linear prestige (current production)
  Config B: + RD decay (30-day periods)
  Config C: + sqrt prestige transform
  Config D: + both RD decay and sqrt prestige

All features, targets, models, hyperparameters are identical.

Usage:
    cd ml && python -m src.benchmark_glicko_signal
"""
...
# ── Feature sets (4-track split = current production) ───────────────
...
# Glicko computation configurations to test
GLICKO_CONFIGS = {
    "A": {"name": "baseline (no decay, linear)", "rd_decay_period": 0, "prestige_transform": "linear"},
    "B": {"name": "+RD decay (30d)", "rd_decay_period": 30, "prestige_transform": "linear"},
    "C": {"name": "+sqrt prestige", "rd_decay_period": 0, "prestige_transform": "sqrt"},
    "D": {"name": "+decay +sqrt", "rd_decay_period": 30, "prestige_transform": "sqrt"},
}


def _stage_feats(stage_type: str) -> list[str]:
    shared = SHARED_BY_TYPE[stage_type]
...
def _merge_glicko_ratings(df: pd.DataFrame, ratings_df: pd.DataFrame) -> pd.DataFrame:
    """Merge all Glicko columns from ratings into feature DataFrame using pre-race lookup."""
    glicko_cols = (
        ["gc_mu", "gc_rd", "stage_mu", "stage_rd"]
        + [f"stage_{st}_mu" for st in GLICKO_STAGE_TYPES]
        + [f"stage_{st}_rd" for st in GLICKO_STAGE_TYPES]
    )

    ratings_sorted = ratings_df.sort_values("race_date")
    glicko_data = []

    for rider_id in df["rider_id"].unique():
        rider_ratings = ratings_sorted[ratings_sorted["rider_id"] == rider_id]
        ...
        for _, row in rider_rows.iterrows():
            ...
            prior = rider_ratings[rider_ratings["race_date"] < race_date] if race_date else pd.DataFrame()

            if len(prior) > 0:
                latest = prior.iloc[-1]
                for col in glicko_cols:
                    entry[col] = latest.get(col, 1500.0 if "mu" in col else 350.0)
                # gc_mu_delta_12m
                cutoff = race_date - pd.Timedelta(days=365)
                older = prior[prior["race_date"] <= cutoff]
                entry["gc_mu_delta_12m"] = latest["gc_mu"] - (older.iloc[-1]["gc_mu"] if len(older) > 0 else 1500.0)
            else:
                for col in glicko_cols:
                    entry[col] = 1500.0 if "mu" in col else 350.0
                entry["gc_mu_delta_12m"] = 0.0

            glicko_data.append(entry)

    glicko_df = pd.DataFrame(glicko_data)

    # Drop existing glicko columns, then merge
    drop_cols = [c for c in glicko_cols + ["gc_mu_delta_12m"] if c in df.columns]
    if drop_cols:
        df = df.drop(columns=drop_cols)

    df = df.merge(glicko_df, on=["rider_id", "race_slug", "year"], how="left")

    for col in glicko_cols:
        df[col] = df[col].fillna(1500.0 if "mu" in col else 350.0)
    df["gc_mu_delta_12m"] = df["gc_mu_delta_12m"].fillna(0.0)

    return df


def _load_base_data():
    """Load cache + stage features + stage targets (no Glicko merge yet)."""
    cache_dfs_raw = []
    ...
    stage_feats_df = pd.read_parquet(os.path.join(CACHE_DIR, "stage_features.parquet"))
    stage_targets = pd.read_parquet(os.path.join(CACHE_DIR, "stage_targets.parquet"))

    df = cache.merge(stage_feats_df, on=["rider_id", "race_slug", "year"], how="left")
    df = df.merge(
    ...
    # Completion rates + sprint pedigree
...
def _green_contender_score(df, completion_rates, sprint_pedigree=None):
    ...
    allround = (
        ...
        + df.get("stage_flat_mu", 0).fillna(0) * 0.005
    )
...
def _run_pipeline(
    df: pd.DataFrame, comp_df: pd.DataFrame, prices_df: pd.DataFrame,
    supply_history, pedigree_by_year: dict,
) -> pd.DataFrame:
    """Run the full source-by-source benchmark pipeline. Returns per-race results."""
    all_race_results = []
    ...
    for fold_num, fold in FOLDS.items():
        ...
        gc_score = (gc_mu_vals - 1.0 * gc_rd_vals) + np.minimum(form * 10, 100)

        test["pred_gc_pts"] = 0.0
        test["pred_gc_daily_pts"] = 0.0
        race_gc_prob = pd.Series(gc_prob, index=test.index)
        race_gc_score = pd.Series(gc_score, index=test.index)
        ...
        for (slug, yr), race_df in test.groupby(["race_slug", "year"]):
            ...
            contenders = race_df[race_gc_prob.loc[race_df.index] >= 0.40].copy()
            contenders["_gc_score"] = race_gc_score.loc[contenders.index]
            if len(contenders) > 0:
                last_pts = gc_table.get(max_pos, 0)
                for pos, (ridx, _) in enumerate(contenders.sort_values("_gc_score", ascending=False).iterrows(), 1):
                    if pos <= max_pos:
                        test.loc[ridx, "pred_gc_pts"] = gc_table.get(pos, 0)
                        test.loc[ridx, "pred_gc_daily_pts"] = estimate_gc_daily_pts(pos, n_stages, race_type)
                    else:
                        beyond = pos - max_pos
                        test.loc[ridx, "pred_gc_pts"] = last_pts * (0.5 ** beyond)
                        test.loc[ridx, "pred_gc_daily_pts"] = estimate_gc_daily_pts(max_pos, n_stages, race_type) * (0.5 ** beyond)
        ...
        for st in STAGE_TYPES:
            ...
            train_exp = train[train[f"n_{st}_stages_ridden"] > 0]
            if len(train_exp) < 10:
                continue
            ...
            if st == "itt":
                ...
                test[f"pred_{st}_stage"] = np.where(gate.predict(X_test) == 1,
                    np.maximum(np.square(mag.predict(X_test)), 0), 0.0)
            else:
                model = Ridge(alpha=1.0)
                model.fit(X_train, y_train)
                test[f"pred_{st}_stage"] = np.maximum(np.square(model.predict(test[avail].fillna(0))), 0)

        test["pred_stage_total"] = sum(test[f"pred_{st}_stage"] * test[f"n_{st}_stages_race"] for st in STAGE_TYPES)
        ...
        test["pred_mtn_final"] = gate_mf.predict_proba(test[avail_mf].fillna(0))[:, 1] * avg_mf
        ...
        if len(has_est) > 0:
            test.loc[has_est.index, "pred_mtn_pass"] = (
                np.maximum(np.square(reg_mp.predict(has_est[avail_mp].fillna(0))), 0) * has_est["estimated_mtn_supply"]
            )

        # ── SPRINT ──────────────────────────────────────────────────
        completion_rates = {}
        for rid in test["rider_id"].unique():
            hist = comp_df[(comp_df["rider_id"] == rid) & (comp_df["year"] < fold["test_year"])]
        ...
        for (slug, yr), idx in test.groupby(["race_slug", "year"]).groups.items():
            ...
            test.loc[idx, "pred_stage_cal"] = _scale_to_supply(test.loc[idx, "pred_stage_total"].values.copy(), supplies["stage"])
            mtn_cal = _sharpen(test.loc[idx, "pred_mountain_raw"].values.copy(), power=2.0, zero_percentile=60)
            test.loc[idx, "pred_mountain_source"] = _scale_to_supply(mtn_cal, supplies["mountain"])
            test.loc[idx, "pred_sprint_source"] = _scale_to_supply(test.loc[idx, "pred_sprint_raw"].values.copy(), supplies["sprint"])

        test["pred_total"] = test["pred_gc_source"] + test["pred_stage_cal"] + test["pred_mountain_source"] + test["pred_sprint_source"]
        test["actual_gc_source"] = test["actual_gc_pts"].fillna(0)
        test["actual_mountain_source"] = test["actual_mountain_final_pts"].fillna(0) + test["actual_mountain_pass_pts"].fillna(0)
        test["actual_sprint_source"] = test["actual_sprint_final_pts"].fillna(0) + test["actual_sprint_inter_pts"].fillna(0)

        # ── EVALUATE ────────────────────────────────────────────────
        for (slug, yr), race_df in test.groupby(["race_slug", "year"]):
            if len(race_df) < 3:
                continue
            result = {"race_slug": slug, "year": yr, "race_type": race_df["race_type"].iloc[0], "fold": fold_num}
            ...
            result["ndcg_20"] = ndcg_at_k(race_df["pred_total"].values, race_df["actual_pts"].values, 20)
            result["p_at_15"] = precision_at_k(race_df["pred_total"].values, race_df["actual_pts"].values, 15)

            race_prices = prices_df[(prices_df["race_slug"] == slug) & (prices_df["year"] == yr)]
            if len(race_prices) > 0:
                ...
                if actual_team:
                    apt = sum(actual_map.get(r, 0) for r in actual_team)
                    ppt = sum(actual_map.get(r, 0) for r in pred_team)
                    result["team_capture"] = ppt / apt if apt > 0 else np.nan
                    result["has_prices"] = apt > 0
                else:
...
def run_benchmark():
    print("=" * 70)
    print("GLICKO SIGNAL QUALITY A/B BENCHMARK")
    print("  RD decay + prestige reform")
    print("=" * 70)

    # Step 1: Load raw results for Glicko computation
    print("\n[1/4] Loading raw results...")
    results_df, _ = load_data_fast(DB_URL)

    # Step 2: Compute ratings for each Glicko config
    print("\n[2/4] Computing Glicko ratings for 4 configurations...")
    ratings_by_config = {}
    for cfg_name, cfg in GLICKO_CONFIGS.items():
        print(f"\n  Config {cfg_name}: {cfg['name']}...")
        ratings_by_config[cfg_name] = compute_all_ratings(
            results_df,
            rd_decay_period=cfg["rd_decay_period"],
            prestige_transform=cfg["prestige_transform"],
        )

    # Step 3: Load base features (without Glicko — we'll merge per config)
    print("\n[3/4] Loading base features...")
    base_df, comp_df, prices_df, supply_history, pedigree_by_year = _load_base_data()
    print(f"  Dataset: {len(base_df):,} rows")

    # Step 4: Run pipeline for each config
    print("\n[4/4] Running benchmark for each configuration...")
    config_results = {}
    for cfg_name, cfg in GLICKO_CONFIGS.items():
        print(f"\n  Config {cfg_name}: {cfg['name']}...")
        df = _merge_glicko_ratings(base_df.copy(), ratings_by_config[cfg_name])
        config_results[cfg_name] = _run_pipeline(df, comp_df, prices_df, supply_history, pedigree_by_year)
    ...
    header = f"{'Metric':>20s}"
    for cfg_name in GLICKO_CONFIGS:
        header += f"  {cfg_name}: {GLICKO_CONFIGS[cfg_name]['name']:>22s}"
    print(header)
    print("-" * len(header))
    ...
    for rt in ["grand_tour", "mini_tour"]:
        ...
        for src in ["gc", "stage", "mountain", "sprint", "total"]:
            ...
            line = f"    {'ρ_' + src:>18s}"
            for cfg_name in GLICKO_CONFIGS:
                vals = config_results[cfg_name][config_results[cfg_name]["race_type"] == rt][col].dropna()
                line += f"  {vals.mean():>28.3f}" if len(vals) > 0 else f"  {'N/A':>28s}"
            print(line)
        ...
        line = f"    {'Team capture':>18s}"
        for cfg_name in GLICKO_CONFIGS:
            priced = config_results[cfg_name][(config_results[cfg_name]["race_type"] == rt) & (config_results[cfg_name]["has_prices"] == True)]
            line += f"  {priced['team_capture'].mean():>27.1%}" if len(priced) > 0 else f"  {'N/A':>28s}"
        print(line)

    # ── Delta table ─────────────────────────────────────────────────
    print("\n" + "=" * 70)
    print("DELTA vs Config A (baseline)")
    print("=" * 70)

    for rt in ["grand_tour", "mini_tour"]:
        print(f"\n  {rt}:")
        a_res = config_results["A"]
        for cfg_name in ["B", "C", "D"]:
            b_res = config_results[cfg_name]
            deltas = []
            for src in ["gc", "stage", "mountain", "sprint", "total"]:
                col = f"rho_{src}"
                a_val = a_res[a_res["race_type"] == rt][col].dropna().mean()
                b_val = b_res[b_res["race_type"] == rt][col].dropna().mean()
                deltas.append((src, b_val - a_val))

            a_priced = a_res[(a_res["race_type"] == rt) & (a_res["has_prices"] == True)]
            b_priced = b_res[(b_res["race_type"] == rt) & (b_res["has_prices"] == True)]
            tc_delta = b_priced["team_capture"].mean() - a_priced["team_capture"].mean() if len(a_priced) > 0 and len(b_priced) > 0 else 0

            parts = "  ".join(f"ρ_{s}:{d:+.3f}" for s, d in deltas)
            print(f"    {cfg_name} ({GLICKO_CONFIGS[cfg_name]['name']:>22s}):  {parts}  TC:{tc_delta:+.1%}")
