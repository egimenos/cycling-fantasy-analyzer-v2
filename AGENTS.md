...
- **Data cache is a lazy in-memory snapshot.** `state.data_cache` (results_df + startlists_df) is loaded from the DB on the first `/predict` request (`load_data` pulls **all** startlists) and only invalidated on model hot-reload. Because the snapshot already contains every race's startlist, the `/predict` handler must, on **every** request, reload the requested `(race_slug, year)` via `load_startlist_for_race()` and **replace** its rows in the cached DataFrame — a merge-if-absent policy would let a stale or partial startlist already in the snapshot shadow the fresh field the API just wrote. Any new table that the API writes to between ML requests needs the same treatment — don't assume the cache is in sync with the DB.
- **The `/analyze` field is the GMV price list, not a scraped PCS startlist.** The frontend imports the fresh price list on demand and posts it to `/analyze`; the use case matches those riders and upserts them (via `startlistRepo.replaceForRace`) as the authoritative `startlist_entries` for the race, which is exactly what the ML scores. PCS startlist scraping (`FetchStartlistUseCase`) is **benchmark-only** (no price list there). The API-side `ml_scores` cache is only served when it covers the current field; otherwise the race is re-predicted (a grown/finalized startlist must not return `null` for the newcomers).
