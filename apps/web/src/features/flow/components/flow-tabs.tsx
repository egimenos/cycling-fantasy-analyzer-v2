...
export function FlowTabs({ activeTab, onTabChange }: FlowTabsProps) {
  ...
  return (
    <div className="relative">
      <div className="flex gap-0 border-b border-outline-variant/10 bg-surface-container-low px-3 md:px-6 overflow-x-auto scrollbar-none">
        {FLOW_STEPS.map((step, i) => {
          ...
          return (
            <button
              ...
              className={cn(
                'px-3 md:px-5 py-3 md:py-3.5 text-[10px] md:text-xs font-mono uppercase tracking-widest transition-all relative flex items-center gap-2 md:gap-2.5 whitespace-nowrap flex-shrink-0',
      ...
      <div className="absolute right-0 top-0 bottom-px w-8 bg-gradient-to-l from-surface-container-low to-transparent pointer-events-none md:hidden" />
    </div>
  );
}
