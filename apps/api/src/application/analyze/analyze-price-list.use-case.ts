...
export class AnalyzePriceListUseCase {
  ...
  private async fetchMlPredictions(
    input: AnalyzeInput,
    riderIds: string[],
  ): Promise<Map<string, { score: number; breakdown: { gc: number; stage: number; mountain: number; sprint: number } | null }> | null> {
    const isStageRace =
    ...
    if (cached.length > 0) {
      ...
      return new Map(cached.map((s) => [s.riderId, {
        score: s.predictedScore,
        breakdown: (s.gcPts || s.stagePts || s.mountainPts || s.sprintPts)
          ? { gc: s.gcPts ?? 0, stage: s.stagePts ?? 0, mountain: s.mountainPts ?? 0, sprint: s.sprintPts ?? 0 }
          : null,
      }]));
    }
    ...
    return new Map(predictions.map((p) => [p.riderId, {
